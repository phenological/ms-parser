...
readSCFA <- function(file, optns = list()){
  ...
  rawData$AnalyteName <- str_trim(str_replace_all(rawData$AnalyteName, c(
                                                   "^(\\d+)" = "\\1-",
                                                   "\\(.*?\\)" = "",
                                                   "Acid" = "acid",
                                                   "\\b[0-9]+C[0-9]+\\b" = "[IS]",
  ...
  rawData$AnalyteName <- str_trim(str_replace_all(rawData$AnalyteName, c("\\d+$" = "")))
